...
const (
	...
	CodePlotReleasePrecondition = 2010 // 地块释放前置条件不足（计划未完成/无收成记录）
	CodePlotStateConflict       = 2011 // 地块已被并发释放，当前状态不允许重复释放
)
...
var ErrorText = map[int]string{
	...
	CodePlotReleasePrecondition: "地块释放条件不足：需存在已完成的种植计划且至少一条对应收成记录",
	CodePlotStateConflict:       "地块已被释放或状态已变化，请刷新后重试",
}
